...
edit ed@Editor {..} ev = case ev of
  ...
  MoveRight ->
    if cursorX == (numCh - 1, EPCol1)
      then ed {cursorX = (0, NoteCol)}
      else ed {cursorX = next cursorX}
  MoveLeft ->
    if cursorX == (0, NoteCol)
      then ed {cursorX = (numCh - 1, EPCol1)}
      else ed {cursorX = prev cursorX}
  JumpUp ->
    if y1 == 0
      then ed {cursorY = (numPat - 1, 0)}
  ...
  JumpRight ->
    if x1 == numCh - 1
      then ed {cursorX = (0, x2)}
      else ed {cursorX = (x1 + 1, x2)}
  JumpLeft ->
    if x1 == 0
      then ed {cursorX = (numCh - 1, x2)}
      else ed {cursorX = (x1 - 1, x2)}
  SelectOctave i ->
    if i < 10
      then ed {octave = i}
 ...
 where
  numRows = length $ snd $ patterns sqncr !! y1
  ...
  rr (x : xs) = if null xs then [x] else init (x : xs)

modifyCurrentCell :: Editor -> (Cell -> Cell) -> Editor
modifyCurrentCell ed@Editor {..} f =
  ed
    { sqncr =
        sqncr
          { patterns = updateNth y1 (second (updateNth y2 (updateNth x1 f))) (patterns sqncr)
          }
    }
 where
  (x1, x2) = cursorX
...
patternEdit ed@Editor {..} c =
 ...
 where
  hexedit f = if isHexDigit c then modifyCurrentCell ed f else ed
